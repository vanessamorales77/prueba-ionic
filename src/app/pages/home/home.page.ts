...
import { IonHeader, IonToolbar, IonTitle, IonContent, IonItem, IonInput, IonButton, IonIcon, IonLabel, IonList, IonCheckbox, IonSelect, IonSelectOption } from '@ionic/angular/standalone';
import { addIcons } from 'ionicons';
import { addOutline, trashOutline, closeOutline, checkmarkOutline, createOutline } from 'ionicons/icons';
...
export class HomePage {
  ...
  addCategory() {
    if (!this.existsCategory(this.newCategory)) {
      ...
      this.actionSheetService.presentInfo('¡Categoría ya existe!', 'La categoría ya existe en la lista');
    }
  }
  ...
  deleteCategory(category: Category) {
    this.actionSheetService.presentDeleteConfirm(category.name, () => {
  ...
  private existsCategory(name: string) {
    return this.categories.find((item: Category) => item.name.toLowerCase().trim() === name.toLowerCase().trim());
  }
